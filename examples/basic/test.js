...
test.only("Click an array of labels and then check their states", async (t) => {
    for (const feature of page.featureList) {
        await t.click(feature.label).expect(feature.checkbox.checked).ok();
    }
});

test.only("Dealing with text using keyboard", async (t) => {
    await t
...
test("Moving the slider", async (t) => {
    ...
    await t
        ...
        .expect(page.slider.handle.offsetLeft)
        .gt(initialOffset);
});
...
test("Handle native confirmation dialog", async (t) => {
    await t.setNativeDialogHandler(() => true).click(page.populateButton);

    const dialogHistory = await t.getNativeDialogHistory();

    await t
        .expect(dialogHistory[0].text)
        .eql("Reset information before proceeding?");

    await t
        .click(page.submitButton)
        .expect(page.results.innerText)
...
test("Pick option from select", async (t) => {
    await t
        ...
        .expect(page.interfaceSelect.value)
        .eql("Both");
});
